...
class AIService {
  ...
  async generateSuggestion(callId, transcriptHistory, documentContext = '', userPreferences = {}) {
    try {
      if (!this.isEnabled) {
        return {
          suggestion: "AI suggestions unavailable - No OpenAI API key configured",
          type: "unavailable",
          confidence: 0,
          reasoning: "OpenAI API key is not configured in the environment variables",
          priority: 0
        };
      }
    ...
    try {
      if (!this.isEnabled) {
        return {
          ...
          engagement_level: 0,
          key_topics: ["AI analysis unavailable - No OpenAI API key configured"],
          objections_raised: [],
  ...
  async processDocumentForContext(documentContent, documentType) {
    try {
      if (!this.isEnabled) {
        return {
          key_features: ["AI document processing unavailable - No OpenAI API key configured"],
          benefits: [],
          pricing_info: [],
          use_cases: [],
          competitive_advantages: [],
          common_objections: [],
          summary: "Document processing unavailable. Please configure an OpenAI API key to enable this feature."
        };
      }
  ...
  async generateMeetingSummary(transcriptHistory, meetingData) {
    try {
      if (!this.isEnabled) {
        return {
          executive_summary: "AI meeting summary unavailable - No OpenAI API key configured",
          key_points: ["OpenAI API key is required for AI-powered meeting summaries"],
          decisions_made: [],
          action_items: [],
          follow_up_items: [],
          sentiment_analysis: "unavailable",
          next_steps: ["Configure OpenAI API key to enable AI meeting summaries"]
        };
      }
